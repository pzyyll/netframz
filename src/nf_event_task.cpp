...
void IOTask::Start(EventLoop &loop, TaskPtr io_task) {
  loop.SetIOTask(io_task);
}

void IOTask::Stop(EventLoop &loop, TaskPtr io_task) {
...
void IOTask::Process(EventLoop &loop, TaskPtr io_task, int mask) {
